...
const defaultDays = dayLabels.map((day) => ({
  day,
  active: false,
  today: false,
}));
...
const StreakCard = ({ dashboard }) => {
  ...
  const mondayFirstTodayIndex = todayIndex === 0 ? 6 : todayIndex - 1;

  const days = weeklyStreak.map((item, index) => ({
    day: dayLabels[index] || item.day,
    active: Boolean(item.active),
    fire: index === mondayFirstTodayIndex && item.active,
  }));
