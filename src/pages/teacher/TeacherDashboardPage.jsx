  ...
  return (
    <main className="min-h-screen bg-white">
      <div className="mx-auto min-h-screen w-full max-w-[460px] bg-white px-[14px] pb-[104px] pt-[49px]">
        <header>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-[11px]">
              <span className="flex h-[27px] items-center rounded-full border border-[#E5E7EB] bg-white px-[14px] text-[14px] font-medium text-black">
                Teacher
              </span>
        ...
        <section className="mt-[18px] rounded-[18px] border border-[#E4D3FF] bg-gradient-to-br from-[#F8F2FF] to-[#FFFFFF] shadow-[0_12px_30px_rgba(101,29,255,0.08)]">
          <div className="relative px-[17px] py-[15px]">
            <div className="relative z-10">

              <h3 className="mt-[8px] text-[36px] font-bold leading-[0.8] tracking-[-0.07em] text-[#191A7A]">
                Octa
              </h3>

              <p className="mt-[4px] text-[11px] font-bold text-[#3918D5]">
                si Gurita Pintar
              </p>

              <p className="mt-[12px] w-[220px] text-[14px] font-bold leading-[1.08] text-black">
                Pantau progress siswa dan bantu mereka berkembang setiap hari.
              </p>
            </div>

            <div className="absolute right-[7px] top-[11px] h-[105px] w-[128px] rounded-full bg-[#EEE4FF] animate-pulse" />

            <img
              src={mascot}
              alt="Octa"
              className="absolute bottom-[0px] right-[1px] z-10 w-[165px] object-contain transition duration-300 hover:scale-105"
            />
          </div>
        </section>
        ...
        {loading ? (
          ...
          <>
            <section className="mt-[17px] grid grid-cols-4 gap-[6px]">
              {summaryCards.map((item) => (
                <SummaryCard key={item.label} item={item} />
              ))}
            </section>

            <section className="mt-[14px] rounded-[18px] border border-[#E5E7EB] bg-white px-[16px] pb-[24px] pt-[18px] shadow-[0_8px_24px_rgba(0,0,0,0.03)]">
              <div className="flex items-center justify-between">
                <h2 className="text-[25px] font-bold tracking-[-0.04em] text-black">
                  Kelas Monitoring
                </h2>

                <button
                  onClick={() => navigate("/teacher/classes")}
                  className="text-[15px] font-bold text-[#5A16E8]"
                >
                  Lihat semua
                </button>
              </div>
              ...
              <div className="mt-[18px]">
                {classMonitoring.length === 0 ? (
                  ...
                  <div className="space-y-[11px]">
                    {classMonitoring.slice(0, 2).map((item) => (
                      <ClassMonitoringCard
                        key={item.id}
                        item={item}
...
const SummaryCard = ({ item }) => {
  ...
  return (
    <div className="flex h-[124px] flex-col items-center rounded-[12px] border border-[#E5E7EB] bg-white px-[5px] py-[11px] text-center shadow-[0_5px_18px_rgba(0,0,0,0.03)] transition duration-300 hover:-translate-y-[2px] hover:shadow-[0_10px_24px_rgba(0,0,0,0.06)]">
      <div
        className={`flex h-[42px] w-[42px] items-center justify-center rounded-[12px] ${item.bg} ${item.color}`}
      >
        <Icon size={24} strokeWidth={2.2} />
      </div>

      <h3 className="mt-[9px] text-[22px] font-bold leading-none text-black">
        {item.value}
      </h3>

      <p className="mt-[9px] whitespace-pre-line text-[11px] font-medium leading-[1.14] text-[#6B7280]">
        {item.label}
      </p>
    </div>
  );
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        <div>
          <h3 className="text-[15px] font-bold leading-none text-[#101348]">
            {item.className}
          </h3>
        ...
        <p className="rounded-full bg-[#F7F0FF] px-[9px] py-[4px] text-[12px] font-bold text-[#651DFF]">
